...
! --- for TEC: IRI-2020 replaced iri_tec(hstart,hend,istep,...) with IRITEC
call IRITEC(glat, glon, jmag, jf, iyyyy, mmdd, DHOUR+25., &
            alt_km_range(1), alt_km_range(2), 1.0, oarr, TECbot, TECtop)
TECtotal = TECbot + TECtop
oarr(37) = TECtotal  ! (m^-2)
oarr(38) = TECtop / TECtotal * 100.  ! % of total
