...
! --- buffer initialization (space-physics datadir patch)
      datadir1 = datadir
      call read_ig_rz
      call readapf107
! --- end buffer init
...
c include only every second auroral boundary point (MLT=0,1,2..23)
      ...
      oarr(100) = fof2  !< foF2 in MHz (space-physics patch, as in IRI2016)
...
1249            oarr(i)=oar(i) 
     ...
     &                  XHOUR,VBEG,VEND,VSTP,a,OARR,datadir1)
...
1349        oarr(iii)=b(iii,i)
          ...
          call IRI_SUB(JF,JMAG,ALATI,ALONG,IYYYY,MMDD,
     &          XHOUR,HEIGHT,HEIGHT,1.,OUTF,OARR,datadir1)
          if(h_tec_max.gt.50.) then
            call IRITEC(ALATI,ALONG,jmag,jf,iyyyy,mmdd,
     &        xhour,h_tec_min,h_tec_max,1.0,oarr,tecbo,tecto)
	    oarr(37) = tecbo + tecto
