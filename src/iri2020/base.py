...
def IRI(time: str | datetime, altkmrange: list[float], glat: float, glon: float) -> xarray.Dataset:
    ...
    if not impr.files(__package__).joinpath(iri_name).is_file():
        build()
    # %% run IRI
    ...
    if np.all(arr[:, 1] == -1):
        raise ValueError(
            f"IRI returned only fill values for {time}: the date is likely outside the "
            "coverage of data/index/ig_rz.dat and apf107.dat. "
            "Download current index files from https://irimodel.org/indices/"
        )
